...
int main (void)
{
    ...
    // 此处编写用户代码 例如外设初始化代码等
    uint32 gps_bcc_xor_success_count = 0;
    uint32 gps_bcc_xor_error_count = 0;
    ...
    ips200_clear();
    ips200_show_string(0, 16*9, "GPS BCCXOR success");
    ips200_show_string(0, 16*10, "GPS BCCXOR error  ");
    ips200_show_uint(160, 16*9, gps_bcc_xor_success_count, 8);
    ips200_show_uint(160, 16*10, gps_bcc_xor_error_count, 8);
    // 此处编写用户代码 例如外设初始化代码等
    ...
    while(1)
    {
        ...
        if(gps_tau1201_flag)
        {
            ...
            if(!gps_data_parse())          //开始解析数据
            {
                ...
                ips200_show_uint(120,  16 * 0, gps_tau1201.time.year,   4);
                ips200_show_uint(160,  16 * 0, gps_tau1201.time.month,  2);
                ips200_show_uint(200,  16 * 0, gps_tau1201.time.day,    2);
                ips200_show_uint(120,  16 * 1, gps_tau1201.time.hour,   2);
                ips200_show_uint(160,  16 * 1, gps_tau1201.time.minute, 2);
                ips200_show_uint(200,  16 * 1, gps_tau1201.time.second, 2);
                ips200_show_uint(120,  16 * 2, gps_tau1201.state,       5);
                ips200_show_float(120, 16 * 3, gps_tau1201.latitude,    4, 6);
                ips200_show_float(120, 16 * 4, gps_tau1201.longitude,   4, 6);
                ips200_show_float(120, 16 * 5, gps_tau1201.speed,       4, 6);
                ips200_show_float(120, 16 * 6, gps_tau1201.direction,   4, 6);
                ips200_show_uint(120,  16 * 7, gps_tau1201.satellite_used, 5);
                ips200_show_float(120, 16 * 8, gps_tau1201.height,      4, 6);
                ips200_show_uint(160,  16 * 9, ++ gps_bcc_xor_success_count, 8);
            }
            else
            {
