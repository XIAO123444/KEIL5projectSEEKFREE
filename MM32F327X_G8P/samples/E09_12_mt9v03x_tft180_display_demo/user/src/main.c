...
int main (void)
{
    ...
    while(1)
    {
        ...
        {
            // 需要注意 直接显示 188*120 分辨率是显示不下的 会直接进入断言报错
            // 需要注意 直接显示 188*120 分辨率是显示不下的 会直接进入断言报错
            // 需要注意 直接显示 188*120 分辨率是显示不下的 会直接进入断言报错
            ...
            // 灰度图像显示 想要修改显示范围就修改本函数后两个参数 分别是显示宽度和高度
            // 灰度图像显示 想要修改显示范围就修改本函数后两个参数 分别是显示宽度和高度
            tft180_displayimage03x((const uint8 *)mt9v03x_image, 160, 120);
            // 这是 tft180_displayimage03x 调用的真实函数 参数意义在其函数头有详细注释
//            tft180_show_gray_image(0, 0, (const uint8 *)mt9v03x_image, MT9V03X_W, MT9V03X_H, 160, 120, 0);
#else
            // 二值化图像显示 参数意义在其函数头有详细注释
            // 二值化图像显示 参数意义在其函数头有详细注释
            // 二值化图像显示 参数意义在其函数头有详细注释
            tft180_show_gray_image(0, 0, (const uint8 *)mt9v03x_image, MT9V03X_W, MT9V03X_H, 160, 120, BINARIZATION_THRESHOLD);
#endif
