...
// 接入正交编码器连接 1
//      模块管脚            单片机管脚
//      LSB                 ENCODER_1_LSB 宏定义的引脚 默认 B4
//      DIR                 ENCODER_1_DIR 宏定义的引脚 默认 B5
//      GND                 核心板电源地 GND
//      3V3                 核心板 3V3 电源
// 
// 接入正交编码器连接 2
//      模块管脚            单片机管脚
//      LSB                 ENCODER_2_LSB 宏定义的引脚 默认 B6
//      DIR                 ENCODER_2_DIR 宏定义的引脚 默认 B7
//      GND                 核心板电源地 GND
//      3V3                 核心板 3V3 电源
...
int main (void)
{
    ...
    // 此处编写用户代码 例如外设初始化代码等
    encoder_dir_init(ENCODER_1, ENCODER_1_LSB, ENCODER_1_DIR);                  // 初始化编码器模块与引脚 正交解码编码器模式
    encoder_dir_init(ENCODER_2, ENCODER_2_LSB, ENCODER_2_DIR);                  // 初始化编码器模块与引脚 正交解码编码器模式
    pit_ms_init(PIT, 100);
    // 此处编写用户代码 例如外设初始化代码等
